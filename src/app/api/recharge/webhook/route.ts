  ...
  try {
    ...
    if (event.meta.event_name === 'order_created') {
      ...
      if (orderAttributes.status === 'paid') {
        ...
        const customData = orderAttributes.checkout_data?.custom;

        if (!customData || !customData.entity_id || !customData.entity_type) {
          console.error('Missing entity_id or entity_type in webhook custom_data:', customData);
          return NextResponse.json({ error: 'Missing custom data in webhook payload.' }, { status: 400 });
        }
